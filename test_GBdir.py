...
def test_rotation_matrices():
    print("single test")
    print(Rot_matrix(EL=90, AZ=150, coord='H'))
    print(Rot_matrix_healpix(EL=90, AZ=150, coord='H'))
    ...
    print ("my routine")
    st = time.time()
    k=Rot_matrix(EL=ELs, AZ=AZs, coord='H')
    ...
    for EL, AZ in zip(ELs, AZs):
        l.append(Rot_matrix_healpix(EL=EL, AZ=AZ, coord='H'))
    print ("elapsed time = %f" % (time.time() - st))
    l=np.array(l)
...
def test_parangle():
    ...
    st=time.time()
    psi = parallactic_angle(a)
    print('elapsed time for parallactic angle for %d samples = %f' % (N, time.time()-st))
    
    st=time.time()
    psi2 = parallactic_angle_einsum(a)
    print('elapsed time for parallactic angle einsum for %d samples = %f' % (N, time.time()-st))
    
    psi1 = parallactic_angle(a[0])
    print psi[0]
    print psi1
    print psi2[0]
